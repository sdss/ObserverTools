...
def get_client(org_id, token, timeout=20000):
    if ping("sdss5-webapp.apo.nmsu.edu"):
        client = InfluxDBClient(url="http://sdss5-webapp.apo.nmsu.edu:9999",
                                token=token, org=org_id, timeout=timeout)
    else:

...
def get_from_influx(name: str, query_name: str, influx_times,
    ...
    results = influx_fetch.query(q_query, influx_times.min(),
                                 influx_times.max(), verbose=verbose)
    influx_times = []
    values = []
    if len(results) != 0:
...
def gen_summary(times, no_enclosure, verbose):
    ...
    for i in range(len(k_times)//2):
        ...
        for j, encl_change in enumerate(influx_vals["Enclosure"]):
            if encl_change == -1:
                continue
            if not no_enclosure:
                ...
                start = 0
                sci_window = influx_vals["tScience"]
            if len(influx_vals["tEnclosure"]) > j + 1 and not no_enclosure:
                sci_window = sci_window & (sci_window
            ...
            if len(t_sci) != 0:
                end = t_sci[-1] + 15 / 60 / 24  # Science ends 15 minutes
    ...
    summary += f"*=*=*=  Bright time lost to technical issues  : 0\n"
    summary += f"\n"
    ...
    summary += f"*=*=*=  Dark time lost to technical issues    : 0\n"

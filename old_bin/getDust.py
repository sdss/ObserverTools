...
if __name__ == "__main__":
    ...
    data1 = archiver.get('25m:apo:dustb', start, end, interpolation='raw',
                         scan_archives=False)
    data2 = archiver.get('25m:apo:encl25m', start, end, interpolation='raw',
                         scan_archives=False)
    ...
    for i in range(nn):
        tm = data1.times[i]
        encl = archiver.get('25m:apo:encl25m', tm, tm, interpolation='raw',
                            scan_archives=False)
        if encl.values[0] > 0:
            sum = sum + data1.values[i] * 5 / 60.0
        if vBool:
            print(data1.times[i], " %7.0f    %2i    %10.0f" % (
            data1.values[i], encl.values[0], sum))
    print("sum=%6.2f" % (sum))

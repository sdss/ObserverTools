...
parser.add_argument('-l', '--list', help='print bad file names', dest="list",
                    action='store_true')
...
def print_line(j, mjd, nfiles=0, nflats=0, nbad=0, pers='n/a'):
    ss = "%4i  %5i  %4i  %4i  %4i    %5.1f" % (
        j, mjd, nfiles, nflats, nbad, pers)
    print(ss)
...
for j, m in enumerate(mjds):
    ...
    print_line(j, m, nfiles, nflats, nbad, proc)
    if args.list:
...
print("-" * width)
# print "Summary for guider flats  MJDs  %s - %s:"   % (mjdsList[0],
# mjdsList[1])
s1 = "Total flats = %s" % nflatsTot
s2 = "Bad flats = %s" % nbadTot
if nflatsTot != 0:
    pers = nbadTot * 100 / nflatsTot
else:
    pers = 0

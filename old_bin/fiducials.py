...
if __name__ == "__main__":
    ...
    data1 = archiver.get('25m:mcp:rotFiducialCrossing:index', start, end,
    ...
    for i in range(nn):
        tm = data1.times[i]
        degrees = archiver.get('25m:mcp:rotFiducialCrossing:deg', tm, tm,
                               interpolation='raw', scan_archives=False)
        if vBool:
            print(data1.times[i],
                  " %7.0f    %2i" % (data1.values[i], degrees.values[0]))

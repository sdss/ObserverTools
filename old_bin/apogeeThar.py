...
def OneFileFitting(data1, fiber, pRef):
    ...
    fitfunc = lambda p0, x: p0[0] * scipy.exp(
        -(x - p0[1]) ** 2 / (2.0 * p0[2] ** 2))
...
def main(argv=None):
    ...
    # current mjd TAI_UTC =34; sjd1=(time.time() + TAI_UTC) / 86400.0 +
    # 40587.3;  sjd= int (sjd1)

    desc = 'apogee arc Thar check for mjd range'
    parser = argparse.ArgumentParser(description=desc)
    parser.add_argument('-m1', '--mjd1', help='start mjd, default current',
                        default=curSjd() - 1, type=int)
    parser.add_argument('-m2', '--mjd2', default=curSjd(),
                        help='end mjd, default is the same as mjd1', type=int)
    parser.add_argument('-r', '--ref', action="store_true",
                        help='references for  lines')
    parser.add_argument('-c', '--cfull', action="store_true",
                        help='do three fibers [30,150,270], default one [150]')
    parser.add_argument('-f', '--fiber', default=150,
                        help='select one fiber, default [150]', type=int)
    parser.add_argument('-A', '--ditherA', action="store_true",
                        help='dither A, default both A and B')
    parser.add_argument('-B', '--ditherB', action="store_true",
                        help='dither B, default both A and B')
    parser.add_argument('-o', '--outfile', help='output file name')
    parser.add_argument('-s', '--sortList', action="store_true",
                        help='to sort the listing')
    ...
    def sBl(a):
        return "(I%s    X%s    W%s)" % (a, a, a)

    header = "# mjd  nExp  Dth fiber %s   %s   %s" % (
        sBl("a"), sBl("b"), sBl("c"),)
    ...
    if args.ref:
        for line in ['a', 'b', 'c']:
            pp = "p0%s" % line;
            p0 = eval(pp)
            myPrint("# ref-%s:  [%6i,  %7.2f,  %4.2f]" % (
                line, p0[0][0], p0[0][1], p0[0][2]), outfile)

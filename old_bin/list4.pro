...
;;    IDL> bossList, '/data/spectro/55103/'
 ...
 pro list4,mjd
    ...
    f1=FINDFILE(cur_path+'sdR-??-????????.fit.gz')
...
;     print,string(i+1, '(i3)')," ", $
;      strmid(fxpar(hdr,"DATE-OBS"),11,5)," ",$
;      strmid(f2[i],4,11),' ',  $
;       string(fxpar(hdr,"CARTID"),'(i2)')," ", $
;       string(fxpar(hdr,"PLATEID"),'(i4)') , " ", $
;       strmid(fxpar(hdr,"FLAVOR"),0,3)," ",  $
;       strmid(fxpar(hdr,"HARTMANN"),0,3)," ", $
;       string(fxpar(hdr,"EXPTIME"),'(f7.1)') ," ", $
;       colla," ",collb," ",collc, haha

;-     print,string(i+1, '(i3)') + ' ' + strmid(fxpar(hdr,"DATE-OBS"),11,5) + ' ' + strmid(f2[i],4,11) + ' ' +$
;-      string(fxpar(hdr,"CARTID"),'(i2)') + ' ' + string(fxpar(hdr,"PLATEID"),'(i4)') + ' ' + $
...
;-      string(fxpar(hdr,"CARTID"),'(i2)') + ' ' + string(fxpar(hdr,"PLATEID"),'(i4)') + ' ' + $
;-      strmid(fxpar(hdr,"FLAVOR"),0,3) + ' ' + strmid(fxpar(hdr,"HARTMANN"),0,3) + ' ' +$
;-      string(fxpar(hdr,"EXPTIME"),'(f7.1)') + ' ' + colla + ' ' + collb + ' ' + collc + ' ' + fxpar(hdr,"NAME") + ' ' + haha

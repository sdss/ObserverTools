...
def get_manga(mjd1, mjd2, verbose):
...
def manga_str(plates, verbose):
    ...
    print("  - %3s manga-dither exposures  (%d x'N', %d x'S', %s x'E') " % (
        mExp, nd, sd, ed))
    print("          %3s 'NSE' triplets  and %s  orphan exposures" % (
        tripl, orphan))
    print("  - %3s manga 'C' exposures obtained on APOGEE-led co-obs plates" % (
        apExp))
    print("  - addition %s exposures on %s other lead plates:" % (
        other_exposures, other_lead))
    print("")
    ...
    print("  -MaNGA dither-")
    print(
        "Ct   Plate  Nexp Texp  Plate_Name   Plate_Type      Lead            "
        "Observations")
    for plate in sorted(plates.keys()):
        if "MaNGA dither" in plates[plate][2]:
            print("%2s   %5s   %2d   %3d               %s  %-14s  %s  " %
                  (plates[plate][4], plate, plates[plate][0], plates[plate][1],
                   plates[plate][5], plates[plate][2], plates[plate][3],))
    print("")
    print("   -Other lead-")
    print(
        "Ct   Plate  Nexp Texp  Plate_Name   Plate_Type      Lead            "
        "Observations")
    for plate in sorted(plates.keys()):
        if (("MaNGA dither" not in plates[plate][2])
           and "APOGEE lead" not in plates[plate][2]):
            print("%2s   %5s   %2d   %3d               %s  %-14s  %s  " %
                  (plates[plate][4], plate, plates[plate][0], plates[plate][1],
                   plates[plate][5], plates[plate][2], plates[plate][3],))
...
def manga(mjd1, mjd2, verbose):
    ...
    print("------------------------------------------------------------------")
    plates = get_manga(mjd1, mjd2, verbose)
    if plates is None:

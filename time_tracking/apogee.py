...
Updates: 2019-05-31 DO added lines 105 and 106 in order for the new dither
positions to reflect 'A' and 'B' correctly. 2019-10-09 EM  Added the
completeness of apogee plates using authoschedule. Consuted with John Donnor.
"""
...
def getApogee(mjd1, mjd2, verbose):
    ...
    for mjd in range(mjd1, mjd2 + 1):
        ...
        if verbose:
            ...
            print(
                "mjd   expNum    imtype  ct plate  Nrd   Dith  Plate_type     "
                " Lead")

        nexp = 0
        for i, f in enumerate(files):
            # print i, f
            ind = re.search(regex, f)
            expNum = f[ind.start():ind.end()]
            fl = grepfitsLib.grepfitsPro(['',
                                          'EXPTYPE,CARTID,PLATEID,NFRAMES,'
                                          'DITHPIX,PLATETYP,SRVYMODE',
                                          f])
            ...
            plate = int(PLATEID)
            NFRAMES = fl[1][4].strip()
            nframes = int(NFRAMES)
            DITHPIX = fl[1][5].strip()
            dithpix = float(DITHPIX)
            PLATETYP = fl[1][6].strip()
            SRVYMODE = fl[1][7].strip()
            lead = SRVYMODE  # [:6]
            ...
            if verbose:
                print("%5s %8s  %-7s %2s %5s   %2d   %1s     %-14s  %s" %
                      (mjd, expNum, exptype, CARTID, plate, nframes, dith,
                       PLATETYP, lead))
...
def ap_str(plates, mjd1, mjd2):
    ...
    bd = 0
    pairs = 0
    orphan = 0  # number of A, B, pairs and orphan
    dblPairs = 0
    dblOrp = 0  # pairs and orphan  for DBA
    mPlates = []  # list of other lead plates
    mExp = 0  # total number of exposures for other plates
    dblExp = 0  # do not use for now, reserved for DBA for other lead
    # observations

    for plate in sorted(plates.keys()):
        tp = plates[plate][4]
        ld = plates[plate][2]
        if (tp == "APOGEE-2&MaNGA" and ld == "APOGEE lead") or (
                tp == "APOGEE-2" and ld == "None"):
            ...
            mExp = mExp + plates[plate][
                0]  # total number of exposures for other plates
    ...
    for p in platesD:
        if p.pct() >= 1.0:
            hist = p.hist.split(",")
            lastMjd = int(hist[-2]) - 2400000
            # print p.plateid,  "    history=", p.hist, "  last=", lastMjd,
            # "end =", mjd2
            if lastMjd <= mjd2:
    ...
    print("  - %3d AB-exposure pairs obtained on APOGEE-led plates + %s single"
          " A or B" % (pairs, orphan))
    ...
    print("Plate   Nexp Nread Plate_name           Plate_type        Lead    "
          "      Done? Observations ")
    for plate in sorted(plates.keys()):
        tp = plates[plate][4]
        ld = plates[plate][2]
        if tp == "APOGEE-2&MaNGA" and ld == "APOGEE lead":
            ...
            # donness=plateObj.calculatedCompletionStatus() - not valid for
            # apogee
            if plate in apDone:
                dd = "Done"
            else:
            ...
            print("%5s   %2d    %2d   %-18s   %-15s   %-10s   %s  %s" %
                  (plate, plates[plate][0], plates[plate][1], plateObj.name,
                   plates[plate][4], plates[plate][2], dd, plates[plate][3]))

    print("")
    print(bold, "APOGEE-2", reset)
    print(
        "Plate   Nexp Nread Plate_name           Plate_type        Lead        "
        "  Done?  Observations ")
    for plate in sorted(plates.keys()):
        tp = plates[plate][4]
        if tp == "APOGEE-2":
    ...
    for plate in sorted(plates.keys()):
        ...
        if tp == "APOGEE-2&MaNGA" and ld != "APOGEE lead":
            ...
            print("%5s   %2d    %2d   %-19s  %-15s   %-12s  -      %s" %
                  (plate, plates[plate][0], plates[plate][1],
                   plateObj.name.strip(), plates[plate][4], plates[plate][2],
                   plates[plate][3]))

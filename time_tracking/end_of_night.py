...
def prn_report(mjd1, plates, mangaPlates):
    print("mjd=", mjd1)
    print("Cart-Plate    Name           Apogee        Manga        Lead      "
          "  Done?  ")
    print("-" * 80)
    ...
    for plate in list(plates.keys()):
        ...
        if plate in mangaPlates:
            ...
            # print "manga:", plate, mangaPlates[plate][0],  mangaPlates[plate]
            # [1], mangaPlates[plate][3]
        else:
            mm = " -----     "
        #          print "%2s-%5s   %-13s   %s*AB*%s   %s     %-7s " % \
        #               (plates[plate][5], plate, plateObj.name,
        #               plates[plate][0]/2, plates[plate][1], mm,
        #               plates[plate][2] )
        ss = "%2s-%5s" % (plates[plate][5], plate)  # cart:plate
        ss = "%s     %-13s" % (ss, plateObj.name)  # name
        ss = "%s   %s*AB*%s rd" % (
            ss, plates[plate][0] / 2, plates[plate][1])  # apogee   4*AB*47
